...
pub fn read<T: FromBytes<LEN>, const LEN: usize>(pid: Pid, base: u64) -> Option<T> {
    let mut buf = [0; LEN];
    read_mem(pid, base, 8, &mut buf).ok()?;
    Some(FromBytes::from_le_bytes(buf))
}

fn read_string(pid: Pid, base: u64) -> Option<String> {
    const MAX_STR_LEN: usize = 256;
    let mut buf = vec![0u8; MAX_STR_LEN];
    read_mem(pid, base, MAX_STR_LEN as u64 - 1, &mut buf).ok()?;
    buf.truncate(buf.iter().position(|&x| x == 0).expect("string too long") + 1);
    let cstr = CString::from_vec_with_nul(buf).expect("invalid unicode");
    Some(cstr.to_string_lossy().to_string())
}

fn class_name(pid: Pid, class: u64) -> Option<String> {
    read_string(pid, read(pid, class + 0x40)?)
}

fn class_kind(pid: Pid, class: u64) -> Option<MonoKind> {
    Some(unsafe { mem::transmute(read::<u8, 1>(pid, class + 0x24)? & 0b111) })
}

fn lookup_class(pid: Pid, cache: u64, name: &str) -> Option<u64> {
...
fn class_static_fields(pid: Pid, class: u64) -> Option<u64> {
...
fn class_field_offset(pid: Pid, class: u64, name: &str) -> Option<u32> {
...
fn instance_class(pid: Pid, instance: u64) -> Option<u64> {
...
fn locate_splitter_info(pid: Pid, instance: u64) -> Option<u64> {
    let splitter_instance: u64 = instance_field(pid, instance, "AutoSplitterInfo")?;
    Some(splitter_instance + 0x10)
}

fn read_boxed_string(pid: Pid, instance: u64) -> Option<String> {
    let class = instance_class(pid, instance)?;
...
impl Celeste {
    fn load_from_process(pid: Pid) -> Option<Self> {
    ...
    fn update(&mut self) -> Option<Info> {
        ...
        read_mem(self.pid, self.info, info_size as u64, &mut buf).ok()?;
        ...
        if save_addr != 0 {
            ...
            if asi.chapter == -1 {
                ...
                if areas_arr != 0 {
                    ...
                    let mode_arr = instance_field::<u64, 8>(self.pid, area_stats, "Modes")? + 0x20;
                    self.mode_stats = read(self.pid, mode_arr + asi.mode as u64 * 8)?;
                }
            }
            // println!("Mode stats: {:x}", self.mode_stats);
            if self.mode_stats != 0 {
                let checkpoints_obj = instance_field(self.pid, self.mode_stats, "Checkpoints")?;
            ...
            if !asi.chapter_started || asi.chapter_complete {
                ...
                if instance_class(self.pid, scene)? != self.level_class {
                    ...
                    let byte: u8 = read(
                        self.pid,
                        scene
                            + class_field_offset(self.pid, self.level_class, "InCutscene")? as u64,
                    )?;
                    byte != 0
                }
            }
...
fn main() -> Result<(), &'static str> {
    ...
    let mut celeste = Celeste::load_from_process(pid).expect("Failed to open");
    let mut old = Info::default();
    let mut failed = false;
    loop {
        thread::sleep(time::Duration::from_millis(100));
        let info = match celeste.update() {
            Some(i) => {
                if failed {
                    println!("Reading worked again")
                }
                failed = false;
                i
            }
...
// TODO: try coalescing reads to do it all up front for each update?
